            ...
            <div className="mt-6">
              ...
              <div className="space-y-3 text-gray-300">
                ...
                <div className="grid grid-cols-2">
                  ...
                  <div>{product.specifications?.weave || product.glassType?.type}</div>
                </div>
                <div className="grid grid-cols-2">
                ...
                <div className="grid grid-cols-2">
                  ...
                  <div>{product.specifications?.weight || product.glassType?.thickness}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Minimal Size</div>
                  <div>{product.specifications?.minimalSize || product.dimensions?.standardSizes?.[2]}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Maximal Size</div>
                  <div>{product.specifications?.maximalSize || product.dimensions?.standardSizes?.[0]}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Pricing</div>
                  <div>{product.specifications?.pricing || `€${product.price}`}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Shipping</div>
                  <div>{product.specifications?.shipping || product.deliveryTime}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Certification</div>
                  <div>{product.specifications?.certification || `VAT ${(product.vatRate || 0) * 100}%`}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-4 text-gray-300">
                <div className="grid grid-cols-2">
                  ...
                  <div>{product.model?.application || product.applications.join(', ')}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Usage</div>
                  <div>{product.model?.usage || 'Not recommended for outdoor use'}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="font-medium text-gray-400">Cleaning &amp; Care</div>
                  <div>{product.model?.cleaningCare || 'Soft and Dry methods - Avoid scratching and applying weight - Use of chemicals not recommended as it may damage the surface of the glass'}</div>
                </div>
              </div>
            </div>
          </div>
